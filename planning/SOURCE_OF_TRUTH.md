...
- Left: Assetly logo mark + "**assetly leasing**" (one clickable unit; click scrolls to Hero; hover = subtle opacity shift only, no decorative animation). The wordmark is set in **DM Serif Display**, matching the Footer's brand lockup — see `DECISIONS.md` DEC-017, which supersedes the earlier Inter Tight treatment here, and DEC-044, which restores the "leasing" suffix DEC-017 had dropped. The bar and the Footer (§17) now carry the same lockup.
...
- Left: logo + "**assetly leasing**" (the same lockup as desktop — DEC-017 removed the desktop/mobile wordmark split, DEC-044 set its wording). Right: hamburger icon.
...
2. **Living Plate** — after drawing completes, two motion classes coexist. Ambient motion remains extremely subtle: the complete plate drifts ~2–5px and only the structural junction nodes slowly reposition. Deliberate mechanical motion is reserved for mechanisms already present in the drawing: the journey marker follows the real Scale/Grow path, the building lift travels its shaft and works its doors, the inner crane-chassis hub rotates on its own axis, and the boom/hook executes a sequential payload placement and pickup cycle. These loops begin only after the draw, use mutually non-harmonic periods, and must resolve through logical arrival, hold, transfer and departure beats rather than abstract or overlapping effects. Under reduced motion and at 640px and below, every deliberate loop is disabled: art-bearing mechanisms retain a truthful static resting state, while motion-only journey/payload marks are hidden. There is no cursor parallax (DEC-055). Must read as a working technical illustration, not an animation showcase. No re-drawing loop (DEC-061).
...
The reading order is DEC-052's — Risk of Obsolescence, Tax Treatment, Leverage Impact, Upfront Cash — and the indices a visitor sees are resequenced 01 to 04 in that order (DEC-053), so the numbered list below is also the order the section is read in. This supersedes DEC-045 and DEC-051. Note that the owner's notes address these arguments by §13's earlier numbering, which no longer matches the site. Each argument keeps its own plate, headline and readings. The four are written as one sequence: reduce ownership risk, improve the treatment of recurring rentals, keep financial capacity lighter, preserve upfront capital.

1. **Risk of Obsolescence** — Plate: asset lifecycle (use → ageing → replacement). Headline: "Use what you need. Leave the ownership behind." (DEC-052; `need` is the single emphasised word). Copy: with an operating lease, Assetly carries the resale and obsolescence exposure; with a loan or outright purchase, that risk remains with the customer. Metric: **customer ownership risk — lower is lighter**. The title names the risk and the metric names who carries it, stated once between them rather than repeated in both. No artificial percentage here. Calculator row: Lease **Assetly bears it** / Loan **You bear it** / Purchase **You bear it**. No artificial percentage here.
2. **Tax Treatment** — Plate: ledger-style drawing resolving into one clean recurring rental line. Headline: "A simpler tax treatment." Copy: full rentals are allowable as a deduction; GST input credit continues to apply. DEC-046 withdrew the depreciation block-rate and 180-day usage claims this line first carried; do not reinstate them without a further decision. Calculator row: Lease **Full rental deductible** / Loan **Depreciation + interest** / Purchase **Depreciation only**.
3. **Leverage Impact** — Plate: progressively heavier balance-sheet structure. Headline: "Keep leverage light. Keep capacity available." Copy: preserve bank credit lines and collateral capacity while maintaining a lighter impact on Debt/Equity and Debt/EBITDA compared with loan-funded acquisition. Calculator row: Lease **Minimal impact** / Loan **Raises Debt/Equity** / Purchase **Drains cash** (DEC-052 makes the Lease reading "Minimal impact").
4. **Upfront Cash** — Plate: retained capital, small portion accessed while a larger reserve stays intact. Headline: "Preserve capital. Keep your business moving." Copy: access the assets you need without a large upfront outlay. The margin and full-purchase figures are carried by the calculator row rather than repeated in the copy (DEC-052). Calculator row: Lease **Low** / Loan **10–25% margin** / Purchase **100% upfront**.
...
**Color**: left narrative on Paper; calculator on Field; Bottle reserved for active controls/emphasis. Full token set per §6, plus `--line: rgba(38,38,27,.15)`. The three graph columns carry one tone each, stepped in lightness (DEC-047): Lease **Bottle `#25453A`**, Loan **Moss `#5C5C46`**, Purchase **Khaki `#B1AD77`** with a hairline edge. Lease holding the deepest tone is the same editorial emphasis its label and reading already carry.

**Calculator behavior**: automatic and context-aware, tied to scroll position — not four separate calculators but "one analytical instrument changing perspective." The visual is one persistent three-column bar graph whose Lease/Loan/Purchase fills transition between qualitative `minimal` / `low` / `mid` / `high` tiers as the focused argument changes. These tiers are presentation categories only, mapped to 16% / 30% / 60% / 92% height; they are not financial quantities. `minimal` is DEC-048's fourth step, for a reading that is not merely the lightest of the three but close to nothing at all. The approved outcome text remains visible beneath each bar. Internal text and bar transitions run for 800ms with `cubic-bezier(.22,1,.36,1)`. Reduced motion applies the destination tier immediately.

**Qualitative tier mapping**: Upfront requirement (lower is lighter) = Lease Minimal / Loan Mid / Purchase High (DEC-048); Customer ownership risk (lower is lighter) = Lease Low / Loan High / Purchase High; Deduction breadth (higher is broader) = Lease High / Loan Mid / Purchase Low; Capacity pressure (lower is lighter) = Lease Low / Loan High / Purchase High. No prototype coefficients, currency calculations, asset-value slider, or numeric magnitudes are used.
...
**Section entry/exit**: calculator hidden before the section; as Slide 01 approaches, drawer opens, layout makes room, the first slide's plate draws, calculator shows Risk of Obsolescence. As the visitor approaches Why Us after Slide 04, the calculator slides away and narrative content returns to full width.
...
**Map artwork**: the approved map is real Bengaluru geography, drawn in the shared `Plate` language (DEC-033, superseding DEC-026). OpenStreetMap data for a 6 × 3.9 km window centred on the office's position on Brigade Road is projected and simplified into static path data at authoring time (`content/plates/bengaluru-map.ts`) and drawn as five weighted layers — parks, water, minor streets, secondary roads, through-roads — so the city reads as itself while the page still ships no map SDK, no tile requests, no API key and none of the default-Google-Maps chrome. The ODbL credit is a licence obligation and is rendered beside the map; the marker links to the same coordinates on a real map. No street names or postal address appear in the section. This resolves `OD-08`.
